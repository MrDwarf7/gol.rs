...
#   GOL_VERSION   - version tag to install (default: latest)
#   GOL_INSTALL   - install dir (default: /usr/local/bin or ~/.local/bin)
#   GITHUB_TOKEN                    - avoids API rate limits in CI

set -e

REPO="MrDwarf7/gol.rs"
RELEASES_URL="https://github.com/$REPO/releases"
BIN="gol"
INSTALL_DIR="$GOL_INSTALL:-}"
...
fetch_latest_version() {
    ...
    [ -n "$tag" ] || die "could not resolve latest release. Install via \`cargo install gol\` instead."
    echo "$tag"
}
...
# Artifact name per draft.yml: gol-<target>-<tag>.zip
artifact="gol-${target}-${version}.zip"
...
echo ""
echo "  gol $ver installed. Run \`gol --help\` to get started."